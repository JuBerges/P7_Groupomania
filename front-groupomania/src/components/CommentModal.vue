<template>
    <div class="modal h-screen w-full fixed left-0 top-0 flex justify-center items-center bg-black bg-opacity-50">
        <!-- modal -->
        <div class="dark-bg rounded-lg shadow-lg w-full sm:w-2/3 mx-2 border-2 darkborder">
            <!-- modal header -->
            <header class="px-4 py-2 flex justify-between items-center rounded-t-lg">
                ...
                <button class="text-black close-modal" @click="closeModal()">
                    ...
                    <svg class="h-4 w-4 my-gray" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- modal body -->
            <div id="commentScroll" class="p-3 h-60 overflow-scroll text-white border-t-2 border-b-2 darkborder">
                <p v-if="noComment">Aucun commentaire pour le moment...&#128546;</p>
                <comment-list @refresh-comments="
            getComments();
            decreaseCommentNumber();
          " v-else :key="com" v-for="com in allComments" :commentContent="com.comment" :commentDate="com.created_at" :commentUsername="com.user.username" :commentUserImg="com.user.avatar" :commentId="com.id" :commentUser="com.user"></comment-list>
            </div>
            <form action="javascript:void(0);" class="flex justify-around items-center w-100 p-3" enctype="multipart/form-data">
                <input id="com" class="w-2/3 p-0.5 outline-blue outline-none rounded bg-gray-200 focus:bg-white" type="text" placeholder="Écrivez un commentaire ici..." v-model="comInput" maxlength="255" />
                <button @click.prevent="sendComment()" class="bg-blue-700 hover:bg-blue-700 px-3 py-1 rounded text-white text-sm">
                    Envoyer
                </button>
            </form>
        </div>
